...
def inRestOfList(state, stateList):
    ...
    if(stateList.count(state)>1):
        ...
        return False
    # statelistCopy = stateList.copy()
    # statelistCopy.pop(0)                 #remove the current first element
    #
    # # if(np.any(state[:0]==state)):
    # #     return True
    #
    # if(contained(state,statelistCopy)):
    #     return True
    #
    # return False

# def contained(state,stateList):
#     stateGrid = state[0].sort()
#     stateWords = state[1].sort()
#
#     for statei in stateList:
#         stateiGrid = statei[0].sort()
#         stateiWords = statei[1].sort()
#
#
#
#         if(stateGrid==stateiGrid and stateWords == stateiWords):
#             return True
#
#     return False
...
#     state = sortWordsByLength(state)
#
#     if(state in stateList):                  #cycle: you describe rule, apply it, then backtrack. Don't wanna paply the rule again
#         return 'FAILED-1'
#     if(isDeadend(state)):
#         return 'FAILED-2'
#     if(goal(state)):
...
#         r = random.randint(0, len(ruleSet) - 1)
#         r = ruleSet[r]
#         print(describeRule(r))
#         # stateList = [state] + stateList
#         newState = applyRule(r, state)
#         describeState(newState)
#         newStateList = [newState] + stateList           #add to front of stateList
#         path = backtrack(newStateList)
#         if path != "FAILED-1" and path != "FAILED-2" and path != "FAILED-3" and path != "FAILED-4" and path != "FAILED-5":
#             return path.append(r)
#
